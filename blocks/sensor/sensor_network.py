...
class SensorNetwork:
    ...
    def _build_graph(self) -> nx.Graph:
        ...
        # Add edges for pairs within communication radius
        dx = self.env.dx
    ...
    def coverage(self, sensing_radius: float | None = None) -> float:
        ...
        This answers: "What percentage of the environment can detect gas?"
        Uses Euclidean distance in grid units.
        """
        ...
        non_wall = np.argwhere(~walls).astype(float)  # (M, 3)
        if len(non_wall) == 0:
            return 0.0

        sensor_pos = self.node_positions_array()  # (N, 3)
        dx = self.env.dx

        # For each non-wall cell, check if any sensor is within radius
        covered = 0
